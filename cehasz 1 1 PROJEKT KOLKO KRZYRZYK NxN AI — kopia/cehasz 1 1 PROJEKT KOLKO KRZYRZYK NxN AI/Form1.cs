...
namespace cehasz_1_1_PROJEKT_KOLKO_KRZYRZYK_NxN_AI
{
    public partial class Form1 : Form
    {
        ...
        bool colour = true; //pomoc w konfigurowaniu algorytmu
                ...
                if (textBox1.Text != "" && start == true && (double.Parse(textBox1.Text.ToString()) >= 3))
                {
                    ...
                    for (int row = 0; row < NxN; row++)
                    {
                        ...
                        for (int col = 0; col < NxN; col++)
                        {
                            ...
                            DynamicButton.DragEnter += new DragEventHandler((sender, e) => DynamicButton_DragEnter(sender, e));
                            DynamicButton.DragDrop += new DragEventHandler((sender, e) => DynamicButton_DragDrop(sender, e));
                            DynamicButton.DragDrop += new DragEventHandler((sender, e) => DynamicButton_Click(sender, e));
        ...
        void label10_MouseDown(object sender, MouseEventArgs e)
        {
            DoDragDrop(label10.Text, DragDropEffects.Copy);                                      //drag and drop z labelu
        }

        private void DynamicButton_DragEnter(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(DataFormats.Text)) e.Effect = DragDropEffects.Copy;          //drag and drop z labelu
        }

        private void DynamicButton_DragDrop(object sender, DragEventArgs e)
        {
            Button button = sender as Button;
            button.Text = (string)e.Data.GetData(DataFormats.Text);         //drag and drop z labelu
        }
        ...
        {
            start = true;
            System.Media.SoundPlayer player = new System.Media.SoundPlayer(@"C:\Users\pshad\Desktop\cehasz 1 1 PROJEKT KOLKO KRZYRZYK NxN AI — kopia\resources\nice.wav");
            player.Play();
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (textBox1.Text != "" && textBox1.Text != "1" && textBox1.Text != "2")
                {
                    ...
                    if (controlsum[0] == (trackBar1.Value - CZY_WYGRA) || controlsum[0] == -(trackBar1.Value - CZY_WYGRA))
                    {
                        ...
                        try
                        {
                            if (Btns[i + 1].Name == "0")
                            {
                                WyliczonePole = i + 1;
                                if(colour==true)
                                Btns[WyliczonePole].BackColor = Color.Gray;
                            }
                        }
                        catch
                        {
                        ...
                        try
                        {
                            if (Btns[i - ((trackBar1.Value - CZY_WYGRA))].Name == "0")
                            {
                                ...
                                if (colour == true)
                                Btns[WyliczonePole].BackColor = Color.DarkGray;
                            }
                        }
                        catch
                        {
                ...
                for (int b = 0; b < (NxN); b++)
                {
                    ...
                    for (int i = b; i < (NxN * NxN); i += NxN)
                    {
                        ...
                        if (controlsum[1] == (trackBar1.Value - CZY_WYGRA) || controlsum[1] == -(trackBar1.Value - CZY_WYGRA))
                        {
                            ...
                            try
                            {
                                if (Btns[i + NxN].Name == "0") // tutaj był błąd 26.11.22 
                                {
                                    WyliczonePole = (i + NxN);
                                    if (colour == true)
                                    Btns[WyliczonePole].BackColor = Color.DarkBlue;
                                }
                            }
                            catch
                            {
                        ...
                        if (controlsum[2] == (trackBar1.Value - CZY_WYGRA) || controlsum[2] == -(trackBar1.Value - CZY_WYGRA))
                        {
                            ...
                            try
                            {
                                if (Btns[a + NxN + 1].Name == "0")
                                {
                                    ...
                                    if (colour == true)
                                    Btns[WyliczonePole].BackColor = Color.Pink;
                                }
                            }
                            catch
                            {
                            ...
                            try
                            {
                                if (Btns[a - ((trackBar1.Value - CZY_WYGRA) * (NxN + 1))].Name == "0")
                                {
                                    ...
                                    if (colour == true)
                                    Btns[WyliczonePole].BackColor = Color.DeepPink;
                                }
                            }
                            catch
                            {
                    ...
                    if (controlsum[3] == (trackBar1.Value - CZY_WYGRA) || controlsum[3] == -(trackBar1.Value - CZY_WYGRA))
                    {
                        ...
                        try
                        {
                            if (Btns[a + NxN - 1].Name == "0")
                            {
                                WyliczonePole = a + NxN - 1;
                                if (colour == true)
                                Btns[WyliczonePole].BackColor = Color.Orange;
                            }
                        }
                        catch
                        {
                        ...
                        try
                        {
                            if (Btns[a - ((trackBar1.Value - CZY_WYGRA) * (NxN - 1))].Name == "0")
                            {
                                WyliczonePole = a - ((trackBar1.Value - CZY_WYGRA) * (NxN - 1));
                                if (colour == true)
                                Btns[WyliczonePole].BackColor = Color.DarkOrange;
                            }
                        }
                        catch
                        {
